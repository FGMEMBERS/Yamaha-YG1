...
var loop = func {
	...
	if (gear_prev != gear.getValue()) {
	 ...
	 k = 0;
	 if (running.getValue() == 1) {rpm.setValue(700+8000*elevator.getValue())} else {rpm.setValue(0)};
      }
	...
	if ((gear.getValue() > 1) and (rpm.getValue() < 100) and (clutch.getValue() == 0)) {
	 running.setValue(0);
	}
	
	if ((running.getValue() == 1) and (gear.getValue() > 0)) { 
	 throttle.setValue(1.25*elevator.getValue()+0.1-rpm.getValue()/8500);
	...
	if ((cranking.getValue() == 1) and ((gear.getValue() == 0) or (clutch.getValue == 1)) and (fuel.getValue()>=0.000000086)) {
	 running.setValue(1);	 
	 cranking.setValue(0);
	}
	...
	if (running.getValue() == 1) {
	 if (fuel.getValue() < 0.000000086) {
	  running.setValue(0);
	  }
	 else {
	  fuel_lev = fuel.getValue();
	  fuel.setValue(fuel_lev - (0.9*elevator.getValue()+0.1)*0.000000086);
	 }
	}

	if (running.getValue() == 1) {
	 if (gear.getValue() == -1) {
	  rpm.setValue(200*speed);
	  rev_throttle.setValue(0.9*elevator.getValue()+0.1-rpm.getValue()/8500);
	...
	mbrake = rpm.getValue()/7000-(1.0*elevator.getValue()+0.25*running.getValue());
