...
var loop = func {
	...
	if (gear_prev != gear.getValue()) {
	 ...
	 k = 0;
	 if (running.getValue() == 1) {rpm.setValue(1400+7300*elevator.getValue())} else {rpm.setValue(0)};
      }
	...
	if ((gear.getValue() > 1) and (rpm.getValue() < 1400) and (clutch.getValue() == 0)) {
	 running.setValue(0);
	}
	
	if ((running.getValue() == 1) and (gear.getValue() > 0)) { 
	 throttle.setValue(0.9*elevator.getValue()+0.1-rpm.getValue()/8000);
	...
	if ((cranking.getValue() == 1) and ((gear.getValue() == 0) or (clutch.getValue == 1)) and (fuel.getValue()>=0.00000056)) {
	 running.setValue(1);	 
	 cranking.setValue(0);
	}
	...
	if (running.getValue() == 1) {
	 if (fuel.getValue() < 0.00000056) {
	  running.setValue(0);
	  }
	 else {
	  fuel_lev = fuel.getValue();
	  fuel.setValue(fuel_lev - (0.9*elevator.getValue()+0.1)*0.00000056);
	 }
	}

	if (running.getValue() == 1) {
	 if (gear.getValue() == -1) {
	  rpm.setValue(500*speed);
	  rev_throttle.setValue(0.9*elevator.getValue()+0.1-rpm.getValue()/8000);
	...
	mbrake = rpm.getValue()/8000-(0.75*elevator.getValue()+0.25*running.getValue());
	tbrake =  getprop("/controls/gear/brake-right");
	if ((gear.getValue() != 0) and (getprop("/gear/gear/rollspeed-ms") != 0) and (clutch.getValue() == 0) and (tbrake < mbrake)) {setprop("/controls/gear/brake-left", mbrake)}
